...
module.exports = async function checkCommand(problemName) {
  ...
  if (testCases.length === 0) {
    console.error(
      chalk.red(
        "× 未找到测试用例,可以使用 oi-tool generator 题目 -c 10 生成10个测试样例，前提是您得写好generator 和 validator"
      )
    );
    process.exit(1);
  }
  ...
  if(allPassed)
  {
    ...
    problemStatus.isvalidated.status = true;
    problemStatus.isgenerated.status = true;
    fs.writeFileSync(statusFile, JSON.stringify(problemStatus),"utf-8");
  }
};
